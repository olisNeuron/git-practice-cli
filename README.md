...
- ✅ 分步提示 / 参考答案 / 一键重置 / 跳过
- ✅ 提交图可视化（CLI 文本图 + Web 矢量 SVG 图），支持「我的图 ↔ 目标图」对比
...
Web 界面右侧的「提交图」面板可以在 **图形 / 文本** 两种画法、以及 **我的图 / 目标图**（参考答案的最终形态）之间切换，方便对照学习。

还支持深链接，例如 `http://localhost:3000/?scenario=04-conflict` 会直接打开指定场景。
...
└── web/
    ├── server.js       # HTTP + WebSocket 服务（共享会话 + 实时广播）
    └── public/         # 浏览器前端
        ├── app.js          # 主逻辑
        ├── line-editor.js  # 终端行编辑器（历史 / 光标 / 快捷键）
        ├── graph.js        # 提交图布局与 SVG 渲染
        └── vendor/         # xterm.js 等（已本地化）
```
